...
// Cache defaultClassNames at module level - called once on module load
const defaultClassNames = getDefaultClassNames();

// Extract components outside to avoid nested component definitions
const CalendarRoot = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(
...
const CalendarDayButton = ({
  ...
  // Use ref callback for focus - avoids effect on every render
  const focusRef = (el: HTMLButtonElement | null) => {
    if (modifiers.focused && el) {
      el.focus();
    }
  };
...
const Calendar = React.forwardRef<HTMLDivElement, CalendarProps>(function Calendar(
  ...
  return (
    <DayPicker
      ...
      className={cn(
        'group/calendar bg-background in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent p-3 [--cell-size:--spacing(7)]',
      ...
      components={{
        Root: (rootProps) => <CalendarRoot ref={ref} {...rootProps} />,
